...
import { FontSizeSlider } from "./panels/fontSizeSlider.js";
...
export class Reader {
	constructor(bookPath, _options) {
		...
		this.fontSizeSlider = new FontSizeSlider(this);
	...
	init(bookPath, _options) {
		...
		this.rendition = this.book.renderTo("viewer", {
			width: "100%",
			height: "100%",
			allowScriptedContent: true, // Enable script execution
		});
	...
	applySavedSettings() {
		...
		if (stored) {
			// Merge spread
			if (stored.spread) {
				this.settings.spread = this.defaults(
					this.settings.spread || {},
					stored.spread
				);
			}
			// Merge styles
			if (stored.styles) {
				this.settings.styles = this.defaults(
					this.settings.styles || {},
					stored.styles
				);
			}
